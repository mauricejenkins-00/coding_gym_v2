...
def llm_review_code(problem_text: str, code: str) -> dict | None:
    ...
    try:
        prompt = (
            ...
            'correctness, feedback, efficiency_rating, notes, and authorship. '
            'Do not include any extra text outside the JSON object. '\
            f'Problem: {problem_text}\n\nCode:\n```python\n{code}\n```\n'
            'Evaluate whether the submitted code appears correct for the problem, whether it has obvious efficiency '
            'strengths or weaknesses, and whether the code seems more likely to be written by a human or generated by an AI assistant.\n'
            'For authorship, use these 7 signs to spot LLM-generated code (explain in notes):\n'
            '1. Over-commented or over-explained simple code (e.g., docstrings for trivial functions)\n'
            '2. Overly descriptive or verbose variable names\n'
            '3. Structurally flawless, perfectly modular, or textbook-clean code\n'
            '4. Lack of external context, config, or real-world dependencies (e.g., no .env, no logging, no error handling for real-world mess)\n'
            '5. Solves toy problems, not real-world edge cases or production constraints\n'
            '6. Functions are perfectly balanced, modular, and typed, but lack real-world messiness or shortcuts\n'
            '7. Feels like a mashup of Stack Overflow/tutorial code, not a single developer\n'
            'If several of these signs are present, authorship is likely "ai-generated". If not, use "human". If unsure, use "uncertain".\n'
            'Use values: correctness as "correct" or "incorrect", efficiency_rating as "Excellent", "Good", or "Needs improvement", '
            'and authorship as "human", "ai-generated", or "uncertain".'
        )
...
def evaluate_submission(problem: dict, code: str) -> dict:
    ...
    if llm_result is not None:
        ...
        result['authorship'] = llm_result.get('authorship', result['authorship'])
...
if 'current_problem' in st.session_state:
    ...
    if submit_code:
        ...
        if evaluation.get('syntax_error'):
            ...
            if evaluation.get('authorship') and evaluation.get('authorship') != 'Unknown':
                st.write('**Authorship:**', evaluation.get('authorship').capitalize())
